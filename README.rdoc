...
2. Before the next execution the process will sleep to wait for the remainder of the m second time period to elapse.
...
The amount of time slept before each execution will depend on both the time period m and the average elapsed time of each execution.
...
The inaccuracy of Ruby's sleep method may cause slight problems when trying to rate-limit methods which take less than 0.001 seconds to execute.
