...
The inaccuracy of Ruby's sleep method may cause slight problem with the AveragedThrottle limiting. Specifically, there limiting accuracy can be slightly-off when trying to rate-limit methods which take less than 0.001 seconds to execute.
...
As mentioned above the throttling is accomplish by blocking script execution using sleep. There is no support for dropping or queuing throttled method invocations.
